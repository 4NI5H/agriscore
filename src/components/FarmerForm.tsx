  ...
  const handleCitySearch = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Also try to extract village/district from display_name if possible
        const parts = display_name.split(', ');
        if (parts.length > 0) {
          handleChange('farmer', 'village', parts[0]);
          if (parts.length > 1) handleChange('farmer', 'district', parts[1]);
          if (parts.length > 2) handleChange('farmer', 'state', parts[parts.length - 2]);
        }
      ...
      <div className="bg-white p-10 rounded-3xl border border-gray-100 shadow-sm">
        {step === 0 && (
          <div className="space-y-8 text-center py-10">
            ...
            <p className="text-gray-500">Choose the language you are most comfortable speaking in. You can also use voice to select.</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-3xl mx-auto">
              {LANGUAGES.map((lang) => (
                ...
                >
                  <div className="text-lg mb-1">{lang.label}</div>
                  <div className="text-[10px] uppercase tracking-widest opacity-60">{lang.name}</div>
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mt-12">
              <button
                onClick={() => setStep(1)}
                className="px-10 py-4 bg-emerald-600 text-white rounded-2xl font-bold uppercase tracking-widest text-sm shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all flex items-center gap-2"
              >
                Continue to Form
              ...
              <button
                ...
                className="px-10 py-4 bg-white border-2 border-emerald-600 text-emerald-600 rounded-2xl font-bold uppercase tracking-widest text-sm hover:bg-emerald-50 transition-all flex items-center gap-2"
              >
                <Mic size={18} />
              ...
              <button
                ...
                className={`px-6 py-4 rounded-2xl font-bold uppercase tracking-widest text-xs transition-all flex items-center gap-2 ${
                  isListening ? 'bg-red-50 text-red-600 animate-pulse' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'
                }`}
              >
                {isListening ? <MicOff size={16} /> : <Mic size={16} />}
                {isListening ? 'Listening...' : 'Speak to Select Language'}
              </button>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 uppercase tracking-tighter">Village</label>
                <input
                  type="text"
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none"
                  value={formData.farmer.village}
                  onChange={(e) => handleChange('farmer', 'village', e.target.value)}
                />
              </div>
            </div>
          </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 mb-8">
              <label className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 block">Search by City/Village</label>
              <div className="flex gap-2">
                <input
                  type="text"
                  placeholder="e.g. Patna, Bihar"
                  className="flex-1 px-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none"
                  value={citySearch}
                ...
                <button
                  ...
                  disabled={isSearchingCity || !citySearch}
                  className="px-6 py-3 bg-emerald-600 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all disabled:opacity-50 flex items-center gap-2"
                >
                  {isSearchingCity ? <Loader2 className="animate-spin" size={16} /> : <MapPin size={16} />}
                  Search
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="col-span-2">
                <button
                  onClick={() => fetchWeather(formData.land.latitude, formData.land.longitude)}
                  disabled={weatherLoading}
                  className="w-full py-3 border-2 border-dashed border-emerald-200 text-emerald-700 rounded-2xl text-xs font-bold uppercase tracking-widest hover:bg-emerald-50 transition-all flex items-center justify-center gap-2"
                >
                  {weatherLoading ? <Loader2 className="animate-spin" size={16} /> : <Cloud size={16} />}
        ...
        {step === 6 && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700 uppercase tracking-tighter">Mock CIBIL Score</label>
                <input
                  type="number"
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none"

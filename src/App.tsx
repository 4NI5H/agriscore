...
export default function App() {
  ...
  // Simulate spotty internet connection for the prototype
  useEffect(() => {
    const interval = setInterval(() => {
      setIsOffline(prev => !prev);
    }, 45000); // Toggle every 45 seconds to demonstrate offline capabilities
  ...
  return (
    <div className="min-h-screen bg-[#F4F7F6] flex font-sans text-gray-900 overflow-hidden">
      {/* Premium Dark Sidebar */}
      <aside className="w-72 bg-[#064E3B] text-white flex flex-col fixed h-full z-20 shadow-2xl">
        <div className="p-8">
      ...
      <div className="flex-1 ml-72 flex flex-col h-screen overflow-y-auto">
        {/* Top Header */}
        <header className="h-20 glass-panel border-b border-gray-200/50 flex items-center justify-between px-10 sticky top-0 z-10">
        ...
        <main className="p-10 flex-1 relative">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="h-full"
            >
              {currentPage === 'dashboard' && <Dashboard onFarmerClick={navigateToDetail} onNavigateToList={() => setCurrentPage('list')} />}
              {currentPage === 'list' && <FarmerList onFarmerClick={navigateToDetail} />}
              {currentPage === 'form' && <FarmerForm onComplete={(id) => navigateToDetail(id)} />}
              {currentPage === 'detail' && selectedFarmerId && (
                <FarmerDetail farmerId={selectedFarmerId} onBack={() => setCurrentPage('list')} />
              )}
